...
Benchmark and replication utilities for Gimbal Regression.
...
The intended use is from notebooks or replication scripts, for example:
...
Benchmark results, particularly elapsed runtime, are implementation-
and environment-specific and should not be interpreted as
hardware-independent algorithmic rankings.
"""
...
def morans_I_residuals_fixed_knn(
...
def spatial_block_kfold_indices(
    ...
    dlon = block_size_m / (
        m_per_deg_lon if m_per_deg_lon > EPS else 111_000.0
    )
...
def _residual_weights_post(
    ...
    if mode == "distance":
        tau = (
            float(h_eff_m)
            if np.isfinite(h_eff_m) and h_eff_m > 0
...
def apply_residual_knn(
    ...
    if "resid" not in df.columns:
        if "y" not in df.columns:
            raise ValueError(
                "apply_residual_knn requires 'resid' or both 'y' and 'yhat'."
            )
...
def fit_train_local_and_residual_knn(
    ...
    *,
    ...
    """
    Fit GR on all training locations using grpy.GimbalRegression,
    then optionally apply downstream residual-KNN and global calibration.
    """
    _require_columns(
        train_df,
        {"lat", "lon", "X", "Y"},
    ...
    if (
        "resid" not in train_local.columns
        and {"y", "yhat"}.issubset(train_local.columns)
    ):
        train_local["resid"] = (
            train_local["y"]
            - train_local["yhat"]
    ...
    if do_residual_knn:
        train_local2 = apply_residual_knn(
            train_local2,
            mode=res_mode,
            eta=res_eta,
        )
    ...
    if do_global_calibration:
        try:
            ...
            logger.warning(
                "Global calibration (uncorrected) failed: %s",
                exc,
            )
        ...
        if do_residual_knn:
            try:
                ...
                logger.warning(
                    "Global calibration (corrected) failed: %s",
                    exc,
                )
...
def predict_test_gr(
    ...
    *,
    ...
    if (
        "ix" in train_local.columns
        and "resid" in train_local.columns
    ):
    ...
    for i in range(len(test_df)):
        ...
        east_m, north_m = tangent_plane_deltas_m(
            lat0,
            lon0,
            S_lat,
            S_lon,
        ...
        theta_ij = bearing_angles_from_deltas(
            east_m,
            north_m,
        )
        ...
        if use_ess:
            ...
            w_norm = (
                w_raw / s
                if s > 0.0
                else np.ones_like(w_raw) / len(w_raw)
            )

            # Same ESS definition as the current GR implementation.
            neff_raw = float(
                effective_sample_size(w_norm)
            )
        ...
        z0_s = (
            (0.0 - z_mu_use) / z_sd_use
            if scale_z.lower() != "none"
            else 0.0
        )
        ...
        if use_residual_knn:
            if res_mode == "distance":
                ...
                raise ValueError(
                    "res_mode must be 'distance' or 'tempered_gr'."
                )
        ...
        row = {
            ...
            "condWLS2": (
                float(condWLS2)
                if np.isfinite(condWLS2)
                else np.nan
            ),
            "condWLS3": (
                float(condWLS3)
                if np.isfinite(condWLS3)
                else np.nan
            ),
            "condM3_nor": (
                float(condM3_nor)
                if np.isfinite(condM3_nor)
                else np.nan
            ),
...
def fit_predict_GR(
    ...
    *,
    ...
    """
    Fit GR on the training fold and predict the test fold.
    """

    train_local2, g_uncorr, g_corr = (
        fit_train_local_and_residual_knn(
            ...
            # Retained from the original benchmark protocol.
    ...
    yhat = (
        pred_df["yhat_corr"].to_numpy(dtype=float)
        if use_residual_knn
        else pred_df["yhat"].to_numpy(dtype=float)
    )
...
def fit_predict_OLS(
    ...
    Xte = sm.add_constant(
        test_df["X"].to_numpy(dtype=float)
    )
...
def fit_predict_RK(
    ...
    _require_dependency(
        _HAS_PYKRIGE and _HAS_SKLEARN,
        "pykrige + scikit-learn",
    )
...
def _bisquare_weights(
    ...
    *,
    ...
    return (
        np.ones_like(d) / len(d)
        if s <= eps
        else (w / s)
    )


def _neff_from_normalized_weights(
    w: np.ndarray,
    eps: float = EPS,
) -> float:
    w = np.asarray(
...
def _gwr_predict_numpy_with_diag(
    ...
    *,
    ...
    for t in range(m):
        ...
        neff_list[t] = (
            _neff_from_normalized_weights(w)
        )
        ...
        condWLS2_list[t] = cond_spd(
            XtWX2
        )

        condWLS3_list[t] = cond_spd(
            XtWX3
        )
...
def _mgwr_predict_numpy_pointwise_with_diag(
    ...
    *,
    ...
    if len(bws) != p:
        raise ValueError(
            f"MGWR expected {p} bandwidths, got {len(bws)}."
        )
...
def _local_ridge_predict_numpy_with_diag(
    ...
    *,
    ...
    condWLS2_list = np.empty(
        m,
        dtype=float,
    )

    condWLS2_ridge_list = np.empty(
        m,
        dtype=float,
    )

    condWLS3_list = np.empty(
        m,
        dtype=float,
    )
    ...
    for t in range(m):
        ...
        if ridge_mode == "trace":
            ...
            raise ValueError(
                "ridge_mode must be one of "
                "{'trace', 'diagmean', 'fixed'}."
            )

        condWLS2_ridge_list[t] = (
            cond_ridge_spd(
                XtWX2,
                lam,
            )
...
def fit_predict_LocalRidge(
    ...
    *,
    ...
    (
        ...
        Xte,
        bw_min,
    ) = _prepare_local_regression_design(
        train_df,
        test_df,
    ...
    yhat, pred_diag = (
        _local_ridge_predict_numpy_with_diag(
            coords_tr,
            ytr.reshape(-1),
            Xtr,
            coords_te,
...
def fit_predict_MGWR(
    ...
    *,
    ...
    yhat, pred_diag = (
        _mgwr_predict_numpy_pointwise_with_diag(
            coords_tr,
            ytr.reshape(-1),
            Xtr,
            coords_te,
...
def aggregate_local_diagnostics(
    ...
    """
    Aggregate local conditioning and ESS diagnostics.

    Per-fold summaries are computed within each fold/model combination.
    The overall ("MEAN") summaries are computed directly from all pooled
    prediction locations for each model, rather than by averaging
    fold-level quantiles.

    Models without local conditioning diagnostics are omitted.
    """
    ...
    # Quantiles to retain for condition-number distributions.
    quantiles = {
        "q25": 0.25,
        "q75": 0.75,
        "q90": 0.90,
    ...
    if (
        "condWLS2_ridge" in df.columns
        and df["condWLS2_ridge"].notna().any()
    ):
    ...
    if (
        "condM3_nor" in df.columns
        and df["condM3_nor"].notna().any()
    ):
        condition_columns.append(
            "condM3_nor"
        )

    # Mean, median, and requested upper/lower quantiles.
    ...
    # Overall summaries
    #
    # Compute directly from all prediction points pooled across folds.
    # Do NOT average fold-level quantiles.
    # -------------------------------------------------------------
    diag_df_mean = (
        df.groupby(
            "model",
...
def run_benchmark(
    ...
    *,
    ...
    df = add_xy_m(
        df,
        force_epsg=force_epsg_xy,
    )

    n = len(df)

    if cv_mode == "spatial":
        splits = spatial_block_kfold_indices(
            df,
            n_splits=n_splits,
        ...
        if (
            ...
            logger.warning(
                "Spatial blocking produced "
                "too few non-empty folds; "
                "falling back to random K-fold."
            )

            splits = random_kfold_indices(
                n,
                n_splits=n_splits,
                seed=seed,
            )
